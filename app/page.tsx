...
export default async function Home() {
  ...
  const stats={technologies:TECHNOLOGIES.length,subTechnologies:TECHNOLOGIES.reduce((sum,item)=>sum+item.subTechnologies.length,0),cases:new Set(official.map((item)=>item.caseId)).size,lakes:new Set(official.map((item)=>item.lake)).size,pending:results.filter((item)=>item.status==="待核验").length,demoCases:new Set(results.filter((item)=>item.isDemo).map((item)=>item.caseId)).size};
  const rollups=technologyRollups(visible);
  const indicatorRows=INDICATORS.slice(0,6).map((indicator)=>({indicator,rollup:rollups.find((item)=>item.indicatorCode===indicator.code)}));
  ...
  return <main><SiteHeader active="home"/>
    ...
    <section className="metricGrid" aria-label="系统概览">{[
      [stats.technologies,"一级技术",`${stats.subTechnologies} 项二级技术`],[stats.cases,"正式案例",official.length?`${stats.lakes} 个独立湖泊`:"等待真实数据"],[stats.demoCases,"示例案例","不计入正式统计"],[stats.pending,"待核验项","不会进入自动汇总"]
